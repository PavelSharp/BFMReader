...
namespace br2proj::bfm {
    ...
    //Назначение этих структур - максимально точно передать структуру файла bfm
    struct bfm_header {
    ...
    struct bfm_part {
        ...
        bound_box box;//Нужно уточнения, возможно это не bound box, совпадало частично. 
    };
    ...
    struct bfm_mesh_desc {
        ...
        int32 unkown2;
        int16 part_index;
        vector<int16> unkown2_data;//TODO провести тесты для всех моделей bfm. Возможно, количесвто этих значений сильно ограничено
        int32 unkown3;
        vector<int16> unkown3_data;//TODO провести тесты для всех моделей bfm. Возможно, количесвто этих значений сильно ограничено
        int32 unkown4;
        ...
        static bfm_mesh_desc sern_read(std::istream& is) {
            ...
            if (res.material_index == -842150451) {
                res.material_index = 0;//TODO need more tests. В таком случае, количесвто meshes должно быть равно 1 'Materail index is default(-842150451) but meshes count not 1'
            }
    ...
    //Такое предстовление, когда вместо одной вершины с несколькими привязанными костями и с соотвеющеми весами,
    //используется несколько субточек, при этом на одну субточку приходиться один вес и одна кость
    //применяется в md5mesh
    struct bfm_point {
    ...
    struct bfm_file {
        ...
        //TODO остаются недочитанные данные. Напр., Rayne.BFM
