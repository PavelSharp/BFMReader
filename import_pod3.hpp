...
{
	//Заголовок занимает 288 байт, доказано, так как первый файл всегда имеет смещение 288 
	struct pod3_header {
		...
		fstring<4> ident;
		uint32 checksum;//CRC-32/MPEG-2 для 280 байт этого заголовка
		fstring<80> comment;
		int32 entries_count;
		int32 journal_count;
	...
	struct pod3_file {
		...
		static pod3_file sern_read(std::istream& is) {
			...
			//todo. отдельный метод, который переупорядочивал бы строки, что бы entry[i]=path[i]
